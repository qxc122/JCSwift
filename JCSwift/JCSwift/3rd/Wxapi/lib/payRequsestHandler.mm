...
#define WechatAppID                 @"wx213a2c40841f4cdd"   //支付用的
#define WechatAppSecret             @"24f762767bb04bda614d5268fa7f626c"

#define MCH_IDOther                       @"1496863012"
//商户API密钥，填写相应参数
#define PARTNER_IDOther                   @"af106ctucq216Lfckg216ckg100xgxfx"
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        [self init:WechatAppID  mch_id:MCH_IDOther];
        //设置密钥
        [self setKey:PARTNER_IDOther];
    }
    return self;
}
...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPayWithOrderNum:(NSString *)orderNum
                                   orderPrice:(CGFloat)orderPrice
{
    //订单标题，展示给用户
    NSString *order_name    = @"海捣网";
    //订单金额,单位（分）
    NSString *priceStr = [NSString stringWithFormat:@"%.f",orderPrice * 100];
    NSString *order_price   = priceStr;
    ...
    [packageParams setObject: @"APP001"         forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: orderNum          forKey:@"body"];        //订单描述，展示给用户
//    [packageParams setObject: kWechatPayNotifyURL        forKey:@"notify_url"];  //支付结果异步通知
